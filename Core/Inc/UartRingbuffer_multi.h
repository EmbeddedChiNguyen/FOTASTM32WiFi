...
typedef struct
{
  ...
  volatile unsigned int tail;
	...
	char ReadLine (char *buffer , char *pBufferData);
	void combineAndStore(int *numbers, int resultArray[]);
/* reads the data in the rx_buffer and increment the tail count in rx_buffer of the given UART */
int Uart_read(UART_HandleTypeDef *uart);
int count_string1(char *string, char search);
...
*/
int Copy_upto_rn (uint8_t *string, uint8_t *buffertocopyinto, UART_HandleTypeDef *uart);
int Copy_upto_closed (uint8_t *string, uint8_t *buffertocopyinto, UART_HandleTypeDef *uart);
